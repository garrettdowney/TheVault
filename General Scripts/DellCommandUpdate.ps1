...
else
{
	...
	if ((Get-CimInstance win32_bios).Manufacturer -match 'Dell')
	{
		...
		$a = "/configure -updateSeverity=recommended,critical -updatetype=driver -updateDeviceCategory=network,storage,audio,input -scheduleAction=DownloadInstallAndNotify -autoSuspendBitLocker=enable -outputLog=$setlog -reboot=disabled"
